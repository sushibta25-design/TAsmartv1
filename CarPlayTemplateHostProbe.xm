...
static NSUInteger gTHWGeneration = 0;
...
static void THWCaptureDestination(void){
    UIView *preview=THWVisibleClass(kTHWPreviewClass); if(!preview)return;
    NSMutableArray *texts=[NSMutableArray array]; THWCollectText(preview,texts);
    NSString *name=nil,*address=nil;
    for(NSString *s in texts){
        NSString *a=THWAddressFromText(s);
        if(!address.length&&a.length)address=a;
        if(!name.length&&s.length>=2&&s.length<=100&&!THWGenericText(s)&&!a.length)name=s;
    }
    BOOL changed=NO;
    ...
    if(changed)THWLog(@"cached name=%@ address=%@",gTHWName?:@"<nil>",gTHWAddress?:@"<nil>");
}
...
static void THWPostWeather(double tempC,NSInteger code,double windKmh){
    ...
    [payload writeToFile:kTHWPayloadPath atomically:YES];
    if(!gTHWNotifyToken){int token=0;uint32_t s=notify_register_check(kTHWWeatherNotify,&token);if(s!=NOTIFY_STATUS_OK){THWLog(@"notify_register_check failed=%u",s);return;}gTHWNotifyToken=token;}
    NSInteger t10=(NSInteger)llround(tempC*10.0),w10=(NSInteger)llround(MAX(0.0,windKmh)*10.0);
    uint64_t state=(uint64_t)MAX(0,MIN(65535,t10+1000))|((uint64_t)MAX(0,MIN(255,code))<<16)|((uint64_t)MAX(0,MIN(65535,w10))<<24);
    notify_set_state(gTHWNotifyToken,state);
    uint32_t p=notify_post(kTHWWeatherNotify);
    THWLog(@"session weather posted name=%@ temp=%.1f code=%ld post=%u",gTHWName?:@"Điểm đến",tempC,(long)code,p);
}

static void THWFetchWeatherCoordinate(CLLocationCoordinate2D c, NSUInteger generation){
    if(generation!=gTHWGeneration)return;
    NSString *u=[NSString stringWithFormat:@"https://api.open-meteo.com/v1/forecast?latitude=%.6f&longitude=%.6f&current=temperature_2m,weather_code,wind_speed_10m&timezone=auto",c.latitude,c.longitude];
    [[[NSURLSession sharedSession]dataTaskWithURL:[NSURL URLWithString:u] completionHandler:^(NSData *data,NSURLResponse *response,NSError *netError){
        if(generation!=gTHWGeneration)return;
        if(netError||!data){THWLog(@"weather error=%@; allow retry",netError.localizedDescription?:@"none");gTHWFetching=NO;gTHWSessionHandled=NO;return;}
        NSDictionary *json=[NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        NSDictionary *cur=[json isKindOfClass:NSDictionary.class]?json[@"current"]:nil;
        NSNumber *temp=cur[@"temperature_2m"],*code=cur[@"weather_code"],*wind=cur[@"wind_speed_10m"];
        if(!temp||!code){THWLog(@"weather parse failed; allow retry");gTHWFetching=NO;gTHWSessionHandled=NO;return;}
        THWPostWeather(temp.doubleValue,code.integerValue,wind?wind.doubleValue:0);
        gTHWFetching=NO;
    }]resume];
}

static void THWFallbackOpenMeteoGeocode(NSString *query, NSUInteger generation){
    if(generation!=gTHWGeneration)return;
    NSURLComponents *c=[NSURLComponents componentsWithString:@"https://geocoding-api.open-meteo.com/v1/search"];
    c.queryItems=@[[NSURLQueryItem queryItemWithName:@"name" value:query?:@""],[NSURLQueryItem queryItemWithName:@"count" value:@"1"],[NSURLQueryItem queryItemWithName:@"language" value:@"vi"],[NSURLQueryItem queryItemWithName:@"format" value:@"json"]];
    NSURL *url=c.URL;
    THWLog(@"fallback geocode Open-Meteo query=%@",query?:@"<nil>");
    [[[NSURLSession sharedSession]dataTaskWithURL:url completionHandler:^(NSData *data,NSURLResponse *response,NSError *error){
        if(generation!=gTHWGeneration)return;
        if(error||!data){THWLog(@"fallback geocode error=%@",error.localizedDescription?:@"none");gTHWFetching=NO;gTHWSessionHandled=NO;return;}
        NSDictionary *json=[NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        NSArray *results=[json isKindOfClass:NSDictionary.class]?json[@"results"]:nil;
        NSDictionary *first=[results isKindOfClass:NSArray.class]?results.firstObject:nil;
        NSNumber *lat=[first isKindOfClass:NSDictionary.class]?first[@"latitude"]:nil;
        NSNumber *lon=[first isKindOfClass:NSDictionary.class]?first[@"longitude"]:nil;
        if(!lat||!lon){THWLog(@"fallback geocode no result for %@",query?:@"<nil>");gTHWFetching=NO;gTHWSessionHandled=NO;return;}
        THWLog(@"fallback geocode success %.6f,%.6f",lat.doubleValue,lon.doubleValue);
        THWFetchWeatherCoordinate(CLLocationCoordinate2DMake(lat.doubleValue,lon.doubleValue),generation);
    }]resume];
}

static void THWFetchForCurrentSession(void){
    if(gTHWFetching||gTHWSessionHandled)return;
    NSString *query=gTHWName.length?gTHWName:gTHWAddress;
    if(!query.length){THWLog(@"navigation started but destination missing");return;}

    gTHWFetching=YES;
    gTHWSessionHandled=YES;
    NSUInteger generation=++gTHWGeneration;
    THWLog(@"new navigation session; geocode name-first=%@ (address=%@)",query,gTHWAddress?:@"<nil>");

    CLGeocoder *geocoder=[CLGeocoder new];
    __block BOOL finished=NO;
    [geocoder geocodeAddressString:query completionHandler:^(NSArray<CLPlacemark *> *placemarks,NSError *error){
        if(generation!=gTHWGeneration||finished)return;
        CLLocation *loc=placemarks.firstObject.location;
        if(loc&&!error){
            finished=YES;
            THWLog(@"CLGeocoder success %.6f,%.6f",loc.coordinate.latitude,loc.coordinate.longitude);
            THWFetchWeatherCoordinate(loc.coordinate,generation);
            return;
        }
        finished=YES;
        THWLog(@"CLGeocoder failed=%@; use Open-Meteo geocoding",error.localizedDescription?:@"none");
        THWFallbackOpenMeteoGeocode(query,generation);
    }];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,4*NSEC_PER_SEC),dispatch_get_main_queue(),^{
        ...
        THWLog(@"CLGeocoder timeout; use Open-Meteo geocoding");
        THWFallbackOpenMeteoGeocode(query,generation);
    });
}

static void THWTick(void){
    THWCaptureDestination();
    BOOL active=THWNavigationActive();
    if(!active){
        if(gTHWLastNavActive||gTHWSessionHandled)THWLog(@"navigation ended; reset session trigger");
        if(gTHWLastNavActive||gTHWSessionHandled){gTHWGeneration++;}
        gTHWSessionHandled=NO;gTHWFetching=NO;
    } else if(!gTHWLastNavActive){
        THWLog(@"navigation start edge detected");
        THWFetchForCurrentSession();
    } else if(!gTHWSessionHandled){
        THWFetchForCurrentSession();
    }
    gTHWLastNavActive=active;
}
...
%ctor{
    @autoreleasepool{
        if(!THWIsHost())return;
        THWLog(@"16.21 sender loaded: name-first geocode + Open-Meteo fallback");
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW,1*NSEC_PER_SEC),dispatch_get_main_queue(),^{
            THWTick();
            [NSTimer scheduledTimerWithTimeInterval:.75 repeats:YES block:^(__unused NSTimer *timer){THWTick();}];
        });
    }
}
