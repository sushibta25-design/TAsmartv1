...
// CarPlayHostBubbleFix.xm — TA Context Lookup Test 16.53
...
static void Build(){
 ...
 @try{
  ...
  Class WL=NSClassFromString(@"UIWindowLayer");
  gRoot=WL ? [WL layer] : [CALayer layer]; gRoot.frame=CGRectMake(0,0,88,88);
  L(@"SOURCE LAYER class=%@",NSStringFromClass(gRoot.class));
  ...
  @try{
    ...
    [gLocal setValue:@"TA16.53" forKey:@"annotation"];
    L(@"PRODUCER DISPLAYABLE=YES options=%@ layer=%@ contentsFormat=%@ displayId=%@ level=%@",
...
%ctor{@autoreleasepool{if(!IsCP())return;L(@"TA WINDOWLAYER CONTEXT TEST 16.53 ACTIVE");dispatch_after(dispatch_time(DISPATCH_TIME_NOW,1*NSEC_PER_SEC),dispatch_get_main_queue(),^{Build();});}}
