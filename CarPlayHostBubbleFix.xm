...
// CarPlayHostBubbleFix.xm — TA Context Lookup Test 16.48
...
static void Build(){
 ...
 @try{
  ...
  @try{
    ...
    [gLocal setValue:@"TA16.48" forKey:@"annotation"];
...
%ctor{@autoreleasepool{if(!IsCP())return;L(@"TA CONTEXT CONTENT TEST 16.48 ACTIVE");dispatch_after(dispatch_time(DISPATCH_TIME_NOW,1*NSEC_PER_SEC),dispatch_get_main_queue(),^{Build();});}}
