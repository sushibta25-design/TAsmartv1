...
// CarPlayHostBubbleFix.xm — TA Context Lookup Test 16.44
...
static void Build(){
 ...
 @try{
  ...
  gHost.frame=CGRectMake(MAX(8.0,CGRectGetWidth(w.bounds)-98),16,88,88);gHost.zPosition=1000000;[w.layer addSublayer:gHost];
...
%ctor{@autoreleasepool{if(!IsCP())return;L(@"TA CONTEXT LOOKUP TEST 16.44 ACTIVE");dispatch_after(dispatch_time(DISPATCH_TIME_NOW,1*NSEC_PER_SEC),dispatch_get_main_queue(),^{Build();});}}
