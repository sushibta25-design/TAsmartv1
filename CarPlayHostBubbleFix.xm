...
// CarPlayHostBubbleFix.xm — TA Context Lookup Test 16.52
...
static void Build(){
 ...
 @try{
  gLocal=((id(*)(id,SEL,id))objc_msgSend)(C,NSSelectorFromString(@"localContextWithOptions:"),@{@"displayId":@2,@"displayable":@YES});
  ...
  @try{
    ...
    [gLocal setValue:@"TA16.52" forKey:@"annotation"];
...
%ctor{@autoreleasepool{if(!IsCP())return;L(@"TA LOCAL CONTEXT OPTIONS 16.52 ACTIVE");dispatch_after(dispatch_time(DISPATCH_TIME_NOW,1*NSEC_PER_SEC),dispatch_get_main_queue(),^{Build();});}}
